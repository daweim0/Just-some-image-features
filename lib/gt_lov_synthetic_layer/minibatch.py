...
def get_minibatch(roidb, voxelizer):
    ...
    # if cfg.MODE == "test":
    image_left_blob, image_right_blob, flow_blob, occluded_blob, left_label_blob, right_label_blob, depth_blob, right_depth_blob, warped_blob = _get_image_blob(
        roidb, random_scale_ind, voxelizer)
    ...
    if cfg.TRAIN.VISUALIZE:
        _vis_minibatch(image_left_blob, image_right_blob, flow_blob, occluded_blob, left_label_blob, right_label_blob,
                       depth_blob, right_depth_blob, warped_blob, roidb)
...
def _vis_minibatch(left_blob, right_blob, flow_blob, occluded_blob, left_label_blob, right_label_blob, depth_blob,
                   right_depth_blob, warped_blob, roidb):
    """Visualize a mini-batch for debugging."""
    for i in range(len(left_blob)):
        ...
        iiiiii = display_img(sintel_utils.sintel_compute_color(np.squeeze(flow_blob[0])), "gt_flow", iiiiii, x_plots,
                             y_plots, fig)
        iiiiii = display_img(sintel_utils.raw_color_from_flow(np.squeeze(flow_blob[0])), "gt_flow", iiiiii, x_plots,
                             y_plots, fig)
