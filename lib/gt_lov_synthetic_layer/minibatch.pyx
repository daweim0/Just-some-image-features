...
cdef _get_image_blob(roidb, scale_ind, voxelizer):
    ...
    for i in xrange(num_images):
        ...
        # im_left_processed_m = im_left.astype(np.float32)
        ...
        for a in prange(shape_0, nogil=True, schedule='guided', num_threads=10):
            ...
            for b in range(shape_1):
                #
                # if im_left_raw_m[a, b, 3] == 0:
                #     im_left_raw_m[a, b, 0] = bg_color_0
                #     im_left_raw_m[a, b, 1] = bg_color_1
                #     im_left_raw_m[a, b, 2] = bg_color_2
                #
                # if im_right_raw_m[a, b, 3] == 0:
                #     im_right_raw_m[a, b, 0] = bg_color_0
                #     im_right_raw_m[a, b, 1] = bg_color_1
                #     im_right_raw_m[a, b, 2] = bg_color_2
...
#         im_left = pad_im(im_left_raw, 16).astype(np.float32)
#
#
#         im_right_raw = cv2.imread(roidb[i]['image_right'], cv2.IMREAD_UNCHANGED)
...
#         im_right = pad_im(im_right_raw, 16).astype(np.float32)
#
#         # meta data
#         # meta_data1 = scipy.io.loadmat(roidb[i*2]['meta_data'])
#         K1 = [[  1.06677800e+03,   0.00000000e+00,   3.12986900e+02],
...
#
#         # depth
#         depth_raw = pad_im(cv2.imread(roidb[i]['depth'], cv2.IMREAD_GRAYSCALE ), 16).astype(np.float32)
#         depth2_processed = pad_im(cv2.imread(roidb[i]['depth_right'], cv2.IMREAD_GRAYSCALE ), 16).astype(np.float32) / 10000
#         processed_depth.append(depth_raw / 10000)
#         processed_depth_right.append(depth2_processed)
#         flow_depth = np.zeros([im_left.shape[0], im_left.shape[1], 2], dtype=np.float32)
#         occluded = np.zeros(flow_depth.shape[:2])
...
#                     occluded_m[a, b] = 1
#
#         # finish processing rgb images
#         im_left = np.asarray(im_left_raw_m).copy()[:, :, :3] - cfg.PIXEL_MEANS
#         im_left_processed = cv2.resize(im_left, None, None, fx=im_scale, fy=im_scale,
#                                        interpolation=cv2.INTER_LINEAR)
#         processed_left.append(im_left_processed)
#         im_right = np.asarray(im_right_raw_m).copy()[:, :, :3] - cfg.PIXEL_MEANS
#         im_right_processed = cv2.resize(im_right, None, None, fx=im_scale, fy=im_scale,
#                                         interpolation=cv2.INTER_LINEAR)
#         processed_right.append(im_right_processed)
#
