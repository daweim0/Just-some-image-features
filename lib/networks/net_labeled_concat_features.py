...
class custom_network(Network):
    ...
    def setup(self):
        ...
        (self.feed('upscale_16x_l', 'skip_link_8x_l')
         ...
         .conv(1, 1, 32, 1, 1, name='8_conv_1', c_i=96 + 128, elu=True)
        ...
        (self.feed('upscale_16x_r', 'skip_link_8x_r')
         .concat(axis=3, name='8_concat')
         .conv(1, 1, 32, 1, 1, name='8_conv_1', c_i=96 + 128, elu=True, reuse=reuse)
        ...
        with tf.device("/cpu:0"):
            ...
            (self.feed(['features_1x_l', 'features_1x_r', 'gt_flow', 'occluded', 'left_labels', 'right_labels'])
             .triplet_flow_loss(margin=1.0, negative_radius=4, positive_radius=1, name="triplet_loss_1x"))

            (self.feed(['features_2x_l', 'features_2x_r', 'gt_flow_2x', 'occluded_2x', 'left_labels_2x', 'right_labels_2x'])
             .triplet_flow_loss(margin=1.0, negative_radius=5, positive_radius=1, name="triplet_loss_2x"))

            (self.feed(['features_4x_l', 'features_4x_r', 'gt_flow_4x', 'occluded_4x', 'left_labels_4x', 'right_labels_4x'])
             .triplet_flow_loss(margin=1.0, negative_radius=5, positive_radius=2, name="triplet_loss_4x"))
